...
//   final GlobalKey<NavigatorState> navigatorKey = GlobalKey<NavigatorState>();
//
//   NotificationService().init(navigatorKey); // 👈 Передаем один и тот же navigatorKey
//
//   runApp(MyApp(navigatorKey: navigatorKey)); // 👈 Передаем в MaterialApp
// }
...
void main() async {
  ...
  NotificationService().init(appRouter); // ✅ Пaередаем router
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      ...
      child: MaterialApp.router(
        ...
        //  onGenerateRoute: (settings) {
        //    print("📌@@@@@@@@@@ onGenerateRoute вызван для ${settings.name}");
        //    if (settings.name == '/newsDetail') {
        //      final String newsId = settings.arguments as String;
        //      return MaterialPageRoute(
        //        builder: (context) => NewsDetailScreen(newsId: newsId),
              ...
              OverlayEntry(
                builder: (_) => BlocBuilder<PhoneAuthCubit, PhoneAuthState>(
                  builder: (context, state) {
                    final user = FirebaseAuth.instance.currentUser;
                    if (state is PhoneAuthSuccess || user != null) {
                      return MainContent(); // ✅ Передаем navigatorKey в MainContent
                    } else {
                      return const PhoneAuthScreen();
                    }
                  },
    ...
    return Scaffold(
      body: _pageController == null
          ...
          : PageView(
              ...
              children: [
                Navigator(
                  //  key: widget.navigatorKey, // ✅ Используем переданный navigatorKey
                  onGenerateRoute: (settings) {
                    print("📌 onGenerateRoute вызван для ${settings.name}");
                    if (settings.name == '/newsDetail') {
                      final String newsId = settings.arguments as String;
